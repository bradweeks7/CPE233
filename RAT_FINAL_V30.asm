...
; registers used: R0, R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11, R12, R13, R14
...
.EQU COLOR_IN = 0x23				; input for current color
...
MAIN:		MOV R3, 0x0E			; initial X coord
			MOV R4, 0x0E			; initial Y coord
			...
			MOV R13, 0x00			; used by update
			MOV R14, 0x00			; used by update
			OUT R8, LEDS_ID
									; get a move 
...
MOVE: 		SEI						; interrupts enabled
			CMP R5, 0x00			; compare new move to previous move
			BREQ MOVE				; if it is the same move, you must try again
...
;---------------------------------------------------------------------
UPDATE:
           MOV   R14, R4         ; copy Y coordinate
           MOV   R13, R3         ; copy X coordinate

           AND   R13, 0x3F       ; make sure top 2 bits cleared
           AND   R14, 0x1F       ; make sure top 3 bits cleared
           LSR   R14             ; need to get the bot 2 bits of r4 into sA
           BRCS  dd_add40A
t1A:       LSR   R14
           BRCS  dd_add80A

dd_outA:   OUT   R13, VGA_LO   ; write bot 8 address bits to register
           OUT   R14, VGA_HI   ; write top 3 address bits to register
           OUT   R9, COLOR  ; write data to frame buffer
           RET

dd_add40A: OR    R13, 0x40       ; set bit if needed
           CLC                  ; freshen bit
           BRN   t1A

dd_add80A: OR    R13, 0x80       ; set bit if needed
           BRN   dd_outA
; --------------------------------------------------------------------
...
;---------------------------------------------------------------------
UPDATE_OLD:
           MOV   R14, R12         ; copy Y coordinate
           MOV   R13, R11         ; copy X coordinate
...
dd_out:    OUT   R13, VGA_LO   ; write bot 8 address bits to register
           OUT   R14, VGA_HI   ; write top 3 address bits to register
           OUT   R10, COLOR  ; write data to frame buffer
           RET

dd_add40:  OR    R13, 0x40       ; set bit if needed
           CLC                  ; freshen bit
           BRN   t1

dd_add80:  OR    R13, 0x80       ; set bit if needed
           BRN   dd_out
; --------------------------------------------------------------------
...
dd_outE:   OUT   R13, VGA_LO   ; write bot 8 address bits to register
           OUT   R14, VGA_HI   ; write top 3 address bits to register
           OUT   R10, COLOR  ; write data to frame buffer
           RET

dd_add40E: OR    R13, 0x40       ; set bit if needed
           CLC                  ; freshen bit
           BRN   t1E

dd_add80E: OR    R13, 0x80       ; set bit if needed
           BRN   dd_outE
; --------------------------------------------------------------------
